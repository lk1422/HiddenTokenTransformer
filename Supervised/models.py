...
#REQUIRES INPUT TO BE PROPERLY PADDED FOR 'ARBITRARY' LENGTH COMPUTATION.
class EncoderArithmetic(nn.Module):
    def __init__(self, d_model, nhead, n_encoder, d_feedforward, max_seq_len, device):
        super().__init__()
        self.transformer = Encoder(d_model, nhead, n_encoder, d_feedforward, max_seq_len, device)
        self.mlp_head = nn.Sequential(*[
                nn.Linear(d_model, d_model),
                nn.ReLU(),
                nn.Linear(d_model, len(TOKEN_LOOKUP))
    ...
    def forward(self, src):
        #src: (S, N, E) 
        out = self.transformer(src) #(T, N, E)
        out = self.mlp_head(out) #(T, N, C)
        return out
...
class Encoder(nn.Module):
    def __init__(self, d_model, nhead, n_encoder, d_feedforward, max_seq_len, device):
        ...
        # Replace learnable positional embeddings with sinusoidal positional encodings
        self.positional_encoding = self.create_sinusoidal_embeddings(max_seq_len, d_model).to(device)
        self.embedding = nn.Embedding(len(TOKEN_LOOKUP), d_model, padding_idx=TOKEN_LOOKUP["<PAD>"])
        encoder = nn.TransformerEncoderLayer(d_model, nhead, d_feedforward)
        self.transformer = nn.TransformerEncoder(encoder, n_encoder)
